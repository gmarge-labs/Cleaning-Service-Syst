...
export function ServiceStep({ data, onUpdate, onNext, onBack, settings }: ServiceStepProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white rounded-xl sm:rounded-2xl border border-neutral-200 shadow-sm p-4 sm:p-6 lg:p-8">
        <div className="mb-4 sm:mb-6 lg:mb-8">
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-neutral-900 mb-1 sm:mb-2">Choose Your Service</h2>
          <p className="text-sm sm:text-base text-neutral-600">Select the type of cleaning service you need</p>
        </div>
            ...
            services.map((service) => {
              ...
              return (
                <div
                  ...
                  className="relative rounded-lg sm:rounded-xl border-2 border-neutral-200 hover:border-secondary-300 bg-white transition-all cursor-pointer"
                >
                  <div className="flex items-start sm:items-center gap-2 sm:gap-3 lg:gap-4 p-3 sm:p-4">
                    {/* Radio Button */}
                    <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 border-neutral-300 flex items-center justify-center flex-shrink-0 transition-all mt-0.5 sm:mt-0">
                    </div>

                    {/* Service Icon */}
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br ${service.color} flex items-center justify-center flex-shrink-0`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                        <h3 className="text-sm sm:text-base font-bold text-neutral-900">{service.name}</h3>
                        {service.popular && (
                          <span className="px-1.5 sm:px-2 py-0.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xs font-semibold rounded-full">
                            Popular
                          </span>
                        )}
                      </div>
                      <p className="text-xs sm:text-sm text-neutral-600 hidden sm:block">{service.description}</p>
                    </div>

                    {/* Price */}
                    <div className="text-right flex-shrink-0">
                      <div className="text-sm sm:text-base font-bold text-neutral-900">${service.price}+</div>
                      <div className="text-xs text-neutral-500 hidden sm:block">per visit</div>
                    </div>
                  </div>
                </div>
              );
